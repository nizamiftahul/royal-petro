...
import { BaseFormRecursive } from './BaseFormRecursive'
import WebField from './web/fields'
import { WFormWrapper } from './web/WFormWrapper'

declare const window: BaseWindow
export const BaseFormTree = (props: IBaseFormProps) => {
  const {
    id,
    parentCtx,
    table,
  ...
  //   data.map((e, idx) => {
  //     const ctx = createContext(state2)
  //     let id = e.id
  //   })
  //   meta.render()
  // }, [state])
    ...
    <>
      {data.length ? (
        <>
          {data.map((e, idx) => {
            console.log('new', e)
            const state2 = { ...state } as any
            const fields = { ...state.config.fields }
            state2.db = {
              ...state2.db,
              data: {},
              config: { ...state.config, fields: fields },
            }

            state2.db.data = { ...e }
            Object.keys(state2.config.fields).map((res) => {
              if (state2.config.fields[res])
                if (state2.config.fields[res].state)
                  if (state2.config.fields[res].state.type === 'datetime') {
                    if (state2.db.data[res]) {
                      state2.config.fields[res].state.value =
                        state2.db.data[res]
                    }
                  }
            })
            meta.manyData[idx] = { state: state2 }
            console.log('fields', state2.config.fields)
            // meta.render()
            const ctx = createContext(meta.manyData[idx].state)
            console.log(meta.manyData[idx].state)
            return (
              <>
                <div
                  className="flex items-stretch justify-between relative items-center align-center p-2"
                  css={css`
                    flex-grow: 0;
                    background: #f3f4f6;
                ...
                <div className="mb-4">
                  <BaseFormRecursive
                    layout={get(meta.manyData[idx].state, 'config.layout')}
                    state={meta.manyData[idx].state}
                    ctx={ctx}
                    direction="col"
                  />
                </div>
