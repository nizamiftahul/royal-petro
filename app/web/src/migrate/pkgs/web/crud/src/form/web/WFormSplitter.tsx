...
export const WFormSplitter = (props: IWFormSplitter) => {
  return (
    ...
    >
      <WFormSplitterInternal
        {...props}
        enabled={props.mode !== 'left' && props.mode !== 'right'}
      />
    </DndProvider>
  )
}
...
const WFormSplitterInternal = ({
  ...
  useEffect(() => {
    ...
    render() // console.log('setting size', meta.size, size)
  }, [size])
    ...
    >
      ...
      {mode !== 'none' && (
        <>
          ...
          <div
            className="flex flex-1 items-stretch self-stretch relative"
            css={css`
              min-height: 45px;
            `}
          >
